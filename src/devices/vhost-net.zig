...
pub const VhostNet = struct {
    ...
    pub fn new(vm: *const Vm, tap_name: [:0]const u8, mac: ?[6]u8, memory: *Memory, mmio_info: MmioDeviceInfo) !Self {
        ...
        virtio_context.avail_features =
            1 << nix.VIRTIO_NET_F_GUEST_CSUM | 1 << nix.VIRTIO_NET_F_CSUM | 1 << nix.VIRTIO_F_VERSION_1 | 1 << nix.VIRTIO_RING_F_EVENT_IDX | 1 << nix.VIRTIO_NET_F_GUEST_TSO4 | 1 << nix.VIRTIO_NET_F_HOST_TSO4 | 1 << nix.VIRTIO_NET_F_GUEST_TSO6 | 1 << nix.VIRTIO_NET_F_HOST_TSO6 | 1 << nix.VIRTIO_NET_F_HOST_USO;
        if (mac) |m| {
